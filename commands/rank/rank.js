...
module.exports = {
    ...
    run: async ({client, message, args}) => {
        ...
        const dataQuery = await schema.findOne({ userID: member, guildID: message.guild.id })
        if (dataQuery) {
            ...
            for (let index in levelRequirements) {
                ...
                if(between(msgcount, levelRequirements[index], levelRequirements[++index]))
                    message.reply({ content: `<@${member}> is level ${index}.` }, { allowedMentions: { users: null}})
            }
        }
        else if (!dataQuery) message.reply(`${member} is level 0.`)
    } 
} 
